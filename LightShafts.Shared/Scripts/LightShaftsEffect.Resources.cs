// Credit: https://github.com/robcupisz/LightShafts

using UnityEngine;

namespace LightShaftsPlugin
{
    public partial class LightShaftsEffect
    {
        public bool m_ForceLightIsOnScreen = true;
        ...
        public DepthShaderType DepthType = DepthShaderType.Opaque;
        ...
        public Shader m_CoordShader = LightShaftsPlugin._Coord;
        Material m_CoordMaterial;

        RenderTexture m_InterpolationEpi;
        public Shader m_DepthBreaksShader = LightShaftsPlugin._DepthBreaks;
        Material m_DepthBreaksMaterial;

        RenderTexture m_RaymarchedLightEpi;
        Material m_RaymarchMaterial;
        public Shader m_RaymarchShader = LightShaftsPlugin._Raymarch;

        RenderTexture m_InterpolateAlongRaysEpi;
        public Shader m_InterpolateAlongRaysShader = LightShaftsPlugin._InterpolateAlongRays;
        Material m_InterpolateAlongRaysMaterial;

        RenderTexture m_SamplePositions;
        public Shader m_SamplePositionsShader = LightShaftsPlugin._SamplePositions;
        Material m_SamplePositionsMaterial;
        bool m_SamplePositionsShaderCompiles = false;

        public Shader m_FinalInterpolationShader = LightShaftsPlugin._FinalInterpolation;
        ...
        void InitLUTs()
        {
            ...
            m_AttenuationCurveTex = new(256, 1, TextureFormat.ARGB32, false, true)
            {
        ...
        void InitRenderTexture(ref RenderTexture rt, int width, int height, int depth, RenderTextureFormat format, bool temp = true)
        {
            ...
            else
            {
                ...
                rt = new(width, height, depth, format)
                {
                    hideFlags = HideFlags.HideAndDontSave
                };
            }
        ...
        void InitSpotFrustumMesh()
        {
            ...
            if (m_SpotMeshNear != m_SpotNear || m_SpotMeshFar != m_SpotFar || m_SpotMeshAngle != l.spotAngle || m_SpotMeshRange != l.range)
            {
                ...
                if (m_SpotMesh.GetTopology(0) != MeshTopology.Triangles || m_SpotMesh.triangles == null || m_SpotMesh.triangles.Length != 36)
                {
                    ...
                    int[] triangles = [ 0, 1, 2, 0, 2, 3, 6, 5, 4, 7, 6, 4, 3, 2, 6, 3, 6, 7, 2, 1, 5, 2, 5, 6, 0, 3, 7, 0, 7, 4, 5, 1, 0, 5, 0, 4 ];
        ...
        bool ShaderCompiles(Shader shader)
        {
            ...
            {
                LightShaftsPlugin._logger.LogError("LightShaftsEffect' " + shader.name + " didn't compile on this platform.");
                return false;
            }
        ...
        public bool CheckMinRequirements()
        {
            ...
            if (!m_MinRequirements)
                LightShaftsPlugin._logger.LogError("LightShaftsEffect require Shader Model 3.0 and render textures (including the RGFloat and RFloat) formats. Disabling.");
            ...
            if (!shadersCompile)
                LightShaftsPlugin._logger.LogError("LightShaftsEffect require above shaders. Disabling.");
